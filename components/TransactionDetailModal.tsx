...
const getStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  detailRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: colors.border, alignItems: 'flex-start' /* Para textos longos */ },
  detailLabel: { fontSize: 16, color: colors.secondaryText, flexShrink: 1, marginRight: 8, flexBasis: '40%' /* Base para o label */ }, 
  detailValue: { fontSize: 16, color: colors.text, fontWeight: '500', flex: 1, textAlign: 'right', flexBasis: '60%' /* Base para o valor */ }, 
...
const TransactionDetailModal: React.FC<TransactionDetailModalProps> = ({
  ...
  const handleDeletePress = () => {
    // O Alert já está na função handleDeleteTransaction em app/(tabs)/home.tsx
    onDelete(transaction.id); 
  };

  const getAmountStyle = () => { if (transaction.type === 'income') return styles.amountValueIncome; if (transaction.type === 'investment') return styles.amountValueInvestment; return transaction.paymentMethod === 'cartao' ? styles.amountValueCard : styles.amountValueExpense;};
  const getPaymentMethodText = () => { if (transaction.type === 'income') return 'Entrada na Conta'; if (transaction.type === 'investment') return 'Movido para Investimentos'; if (transaction.category === "Pagamento de Fatura CC") return 'Pagamento de Fatura (Saldo)'; return transaction.paymentMethod === 'saldo' ? 'Saldo da Conta' : 'Cartão de Crédito';};
  const transactionTypeText = () => { if (transaction.category === "Pagamento de Fatura CC") return "Pagamento de Fatura"; switch(transaction.type) { case 'income': return "Entrada"; case 'expense': return "Saída"; case 'investment': return "Investimento"; default: return "N/A";}};
        ...
        <View style={styles.modalView}>
          <ScrollView contentContainerStyle={styles.scrollViewContent} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailValue} numberOfLines={3} ellipsizeMode="tail">{transaction.description}</Text>
            </View>
            {transaction.notes && (
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>Notas:</Text>
                <Text style={styles.detailValue} numberOfLines={3} ellipsizeMode="tail">{transaction.notes}</Text>
              </View>
            )}
